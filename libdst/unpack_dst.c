...
/* ── 진단용 전역 변수 (처음 N프레임만 상세 출력) ── */
static int  g_dst_frame_cnt = 0;   /* UnpackDSTframe 호출마다 증가    */
static int  g_dst_diag_max  = 15;  /* 이 프레임 번호까지 상세 출력    */
#define DST_DIAG (g_dst_frame_cnt < g_dst_diag_max)
                        ...
                        CodedTable  *CF)
{
  ...
  for(FilterNr = 0; FilterNr < FH->NrOfFilters; FilterNr++)
  {
    ...
    if (DST_DIAG)
      fprintf(stderr, "[COEF] F#%03d Filter[%d] PredOrder=%d Coded=%d bits_so_far=%d\n",
              g_dst_frame_cnt-1, FilterNr, FH->PredOrder[FilterNr], CF->Coded[FilterNr],
              (int)get_in_bitcount(SD));
    ...
    if (CF->Coded[FilterNr] == 0)
    {
      ...
      for(CoefNr = 0; CoefNr < FH->PredOrder[FilterNr]; CoefNr++)
      {
        if (FIO_BitGetShortSigned(SD, SIZE_PREDCOEF, &FH->ICoefA[FilterNr][CoefNr]))
          return DSTErr_NegativeBitAllocation;
        /* 범위 확인: uncoded이므로 9-bit signed는 항상 범위 내 */
      }
      if (DST_DIAG)
        fprintf(stderr, "[COEF] F#%03d Filter[%d] uncoded %d coefs OK, bits=%d\n",
                g_dst_frame_cnt-1, FilterNr, FH->PredOrder[FilterNr],
                (int)get_in_bitcount(SD));
    }
    else
    {
      ...
      if (DST_DIAG)
        fprintf(stderr, "[COEF] F#%03d Filter[%d] Rice: bestmethod=%d CPredOrder=%d bits=%d\n",
                g_dst_frame_cnt-1, FilterNr, bestmethod,
                CF->CPredOrder[bestmethod], (int)get_in_bitcount(SD));
      ...
      for(CoefNr = CF->CPredOrder[bestmethod]; CoefNr < FH->PredOrder[FilterNr]; CoefNr++)
      {
        ...
        /* 범위 검사 제거 — 실제 SACD 상업용 인코더는 Philips 레퍼런스의
         * [-256,255] 범위를 초과하는 계수를 생성할 수 있음.
         * int16_t에는 문제없이 저장 가능하므로 그냥 사용. */
        if (DST_DIAG && ((c < -(1<<(SIZE_PREDCOEF-1))) || (c >= (1<<(SIZE_PREDCOEF-1)))))
          fprintf(stderr, "[COEF] F#%03d OOB(ignored): Filter[%d] Coef[%d] c=%d x=%d m=%d\n",
                  g_dst_frame_cnt-1, FilterNr, CoefNr, c, x,
                  CF->m[FilterNr][bestmethod]);
    ...
    if (FH->PtableLen[PtableNr] > 1)
    {
      ...
      else
      {
        ...
        for(EntryNr = CP->CPredOrder[bestmethod]; EntryNr < FH->PtableLen[PtableNr]; EntryNr++)
        {
          ...
          {
            if (DST_DIAG)
              fprintf(stderr, "[PTBL] F#%03d EntryNr=%d out of bounds (AC_HISMAX=%d) — skip\n",
                      g_dst_frame_cnt-1, EntryNr, AC_HISMAX);
          ...
          /* 범위 초과 시 클램프 — 실제 SACD 인코더 출력 호환성 */
          if (c < 1) c = 1;
          if (c > (1 << (AC_BITS - 1))) c = (1 << (AC_BITS - 1));
                   ...
                   uint8_t* DSDdataframe)
{
  ...
  if (DST_DIAG)
    fprintf(stderr, "[UNPACK] F#%03d sz=%ld DSTCoded=%d\n",
            g_dst_frame_cnt-1, D->FrameHdr.CalcNrOfBytes, D->FrameHdr.DSTCoded);

  if (D->FrameHdr.DSTCoded == 0)
  {
    if (FIO_BitGetIntUnsigned(&D->S, 1, &Dummy))
      return DSTErr_NegativeBitAllocation;
    ...
    {
      if (DST_DIAG)
        fprintf(stderr, "[STUFF] F#%03d DSTCoded=0 stuffing=0x%02x != 0\n",
                g_dst_frame_cnt-1, Dummy);
  ...
  else
  {
    ...
    {
      if (DST_DIAG)
        fprintf(stderr, "[UNPACK] F#%03d ReadSegmentData FAIL err=%d bits=%d\n",
                g_dst_frame_cnt-1, error, (int)get_in_bitcount(&D->S));
      return error;
    }
    if (DST_DIAG)
      fprintf(stderr, "[UNPACK] F#%03d after ReadSegmentData: bits=%d NrOfFilters(pre)=%d\n",
              g_dst_frame_cnt-1, (int)get_in_bitcount(&D->S), D->FrameHdr.NrOfFilters);

    if ((error = ReadMappingData(&D->S, &D->FrameHdr)) != 0)
    {
      if (DST_DIAG)
        fprintf(stderr, "[UNPACK] F#%03d ReadMappingData FAIL err=%d bits=%d\n",
                g_dst_frame_cnt-1, error, (int)get_in_bitcount(&D->S));
      return error;
    }
    if (DST_DIAG)
      fprintf(stderr, "[UNPACK] F#%03d after ReadMappingData: bits=%d NrOfFilters=%d NrOfPtables=%d\n",
              g_dst_frame_cnt-1, (int)get_in_bitcount(&D->S),
              D->FrameHdr.NrOfFilters, D->FrameHdr.NrOfPtables);

    if ((error = ReadFilterCoefSets(&D->S, D->FrameHdr.NrOfChannels, &D->FrameHdr, &D->StrFilter)) != 0)
    {
      if (DST_DIAG)
        fprintf(stderr, "[UNPACK] F#%03d ReadFilterCoefSets FAIL err=%d bits=%d\n",
                g_dst_frame_cnt-1, error, (int)get_in_bitcount(&D->S));
      return error;
    }
    if (DST_DIAG)
      fprintf(stderr, "[UNPACK] F#%03d after ReadFilterCoefSets: bits=%d\n",
              g_dst_frame_cnt-1, (int)get_in_bitcount(&D->S));

    if ((error = ReadProbabilityTables(&D->S, &D->FrameHdr, &D->StrPtable, D->P_one)) != 0)
    {
      if (DST_DIAG)
        fprintf(stderr, "[UNPACK] F#%03d ReadProbabilityTables FAIL err=%d bits=%d\n",
                g_dst_frame_cnt-1, error, (int)get_in_bitcount(&D->S));
      return error;
    }
    ...
    /* 진단: 처음 5프레임의 P_one 테이블 전체 덤프 */
    if (g_dst_frame_cnt <= 5) {
      int pnr, enr;
      fprintf(stderr, "[PTBL_DUMP] F#%03d NrOfPtables=%d ADataLen=%d\n",
              g_dst_frame_cnt-1, D->FrameHdr.NrOfPtables, D->ADataLen);
      for (pnr = 0; pnr < D->FrameHdr.NrOfPtables; pnr++) {
        fprintf(stderr, "  Ptable[%d] len=%d entries:", pnr, D->FrameHdr.PtableLen[pnr]);
        for (enr = 0; enr < D->FrameHdr.PtableLen[pnr] && enr < 64; enr++)
          fprintf(stderr, " %d", D->P_one[pnr][enr]);
        fprintf(stderr, "\n");
      }
      /* AData 첫 32비트 덤프 */
      fprintf(stderr, "  AData[0..31]:");
      for (enr = 0; enr < 32 && enr < D->ADataLen; enr++)
        fprintf(stderr, " %d", (int)D->AData[enr]);
      fprintf(stderr, "\n");
    }

    /* DST spec: 첫 번째 비트(MSB)만 0이어야 함.
     * Philips 원본은 첫 바이트 전체(8비트)를 0x00 체크했으나 과도하게 엄격.
     * SACD 상용 인코더는 첫 비트=0, 나머지 7비트는 임의값 사용 가능. */
    if (DST_DIAG && D->ADataLen > 0)
      fprintf(stderr, "[ACDATA] F#%03d AData[0]=0x%02x bits_after_ptbl=%d\n",
              g_dst_frame_cnt-1, (unsigned char)D->AData[0], (int)get_in_bitcount(&D->S));
    if ((D->ADataLen > 0) && (D->AData[0] & 0x80))
