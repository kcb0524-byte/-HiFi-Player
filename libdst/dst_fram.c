...
static __inline int LT_ACGetPtableIndex(int32_t PredicVal, int PtableLen)
{
    int  j;
...
static void LT_InitCoefTablesI(ebunch *D, int32_t ICoefI[2 * MAX_CHANNELS][16][256])
{
    ...
    for (FilterNr = 0; FilterNr < D->FrameHdr.NrOfFilters; FilterNr++)
    {
        ...
        for (TableNr = 0; TableNr < 16; TableNr++)
        {
            ...
            for (i = 0; i < 256; i++)
            {
                ...
                ICoefI[FilterNr][TableNr][i] = cvalue;  /* int32_t: no truncation for large SACD coefficients */
            }
        }
    }
}
...
int DST_FramDSTDecode(uint8_t *DSTdata, uint8_t *MuxedDSDdata, int FrameSizeInBytes, int FrameCnt, ebunch *D)
{
    ...
    if (error == DSTErr_NoError && D->FrameHdr.DSTCoded == 1)
    {
        ACData AC;
#ifdef _MSC_VER
        __declspec(align(16)) int32_t  LT_ICoefI[2 * MAX_CHANNELS][16][256];
        __declspec(align(16)) uint8_t  LT_Status[MAX_CHANNELS][16];
#else
        int32_t  LT_ICoefI[2 * MAX_CHANNELS][16][256]  __attribute__ ((aligned (16)));  /* int32_t: SACD coefficients can exceed int16_t range */
        ...
        /* 진단: 초기 seed 디코드 결과 */
        if (FrameCnt < 5) {
            fprintf(stderr, "[SEED] F#%03d Reverse7LSBs(ICoefA[0][0]=%d)=%d seed_b=%d cbptr_after=%d\n",
                FrameCnt, D->FrameHdr.ICoefA[0][0], Reverse7LSBs(D->FrameHdr.ICoefA[0][0]), ACError, AC.cbptr);
        }
        ...
        for (BitNr = 0; BitNr < NrOfBitsPerCh; BitNr++)
        {
            ...
            for (ChNr = 0; ChNr < NrOfChannels; ChNr++)
            {
                int32_t Predict;  /* int32_t: SACD filter table entries can exceed int16_t range */
                uint8_t Residual;
                int16_t BitVal;
                ...
                if ((D->FrameHdr.HalfProb[ChNr]/* == 1*/) && (BitNr < D->FrameHdr.NrOfHalfBits[ChNr]))
                {
                    ...
                    /* 진단: 처음 5프레임의 처음 30비트 상세 */
                    if (FrameCnt < 5 && BitNr < 30) {
                        fprintf(stderr, "[BIT] F#%03d b%d ch%d Predict=%d sign=%d HALF_PROB p=%d Res=%d BitVal=%d cbptr=%d\n",
                            FrameCnt, BitNr, ChNr, Predict,
                            (int)(((uint32_t)Predict)>>31),
                            AC_PROBS/2, Residual,
                            (int)((((uint32_t)Predict)>>31) ^ Residual) & 1,
                            AC.cbptr);
                    }
                }
                else
                {
                    ...
                    /* 진단: 처음 5프레임의 처음 30비트 상세 */
                    if (FrameCnt < 5 && BitNr < 30) {
                        fprintf(stderr, "[BIT] F#%03d b%d ch%d Predict=%d sign=%d tbl=%d ptblLen=%d j=%d p_one=%d cbptr_pre=%d\n",
                            FrameCnt, BitNr, ChNr, Predict,
                            (int)(((uint32_t)Predict)>>31),
                            table4bit, D->FrameHdr.PtableLen[table4bit], PtableIndex,
                            D->P_one[table4bit][PtableIndex], AC.cbptr);
                    }

                    LT_ACDecodeBit_Decode(&AC, &Residual, D->P_one[table4bit][PtableIndex], D->AData, D->ADataLen);

                    if (FrameCnt < 5 && BitNr < 30) {
                        int bv = (int)((((uint32_t)Predict)>>31) ^ Residual) & 1;
                        fprintf(stderr, "[BIT] F#%03d b%d ch%d Res=%d BitVal=%d cbptr_post=%d\n",
                            FrameCnt, BitNr, ChNr, Residual, bv, AC.cbptr);
                    }
                }

                /* Channel bit depends on the predicted bit and BitResidual[][] */
                BitVal = ((((uint32_t)Predict) >> 31) ^ Residual) & 1;  /* sign check: use bit31 of int32_t */
        ...
        /* AC flush 진단: cbptr가 ADataLen에서 얼마나 떨어졌는지 출력
         * cbptr ≈ ADataLen  → 정상 (AC 디코더가 모든 비트 소비)
         * cbptr << ADataLen → 필터 예측 오류 등으로 AC가 일찍 수렴
         * gap = ADataLen - cbptr: 양수=부족 소비, 음수=초과 소비 */
        {
            int gap = D->ADataLen - AC.cbptr;
            if (FrameCnt < 30 || ACError != 1)
                fprintf(stderr, "[ACFLUSH] F#%03d cbptr=%d ADataLen=%d gap=%d ACError=%d %s\n",
                    FrameCnt, AC.cbptr, D->ADataLen, gap,
                    ACError, (ACError == 1) ? "PASS" : "FAIL");
        }

        /* ACError=1 → PASS (cbptr >= ADataLen-7)
         * ACError=0 → 원본 Philips 조건 실패 → err15 반환, 디코더 리셋 */
        if (ACError != 1)
